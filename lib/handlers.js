...
const debug = utils.debuglog("performance");
...
const handlers = {
  ...
  _tokens: {
    post: (data, callback) => {
      ...
      if (phone && password) {
        ...
        _data.read("users", phone, (error, data) => {
          _performance.mark("User lookup finished");
          if (!error && data) {
              ...
              if (!err && userData) {
                ...
                if (checks.length < config.maxChecks) {
                  ...
                  dns.resolve(hostname, (err, dnsRecords) => {
                    ...
                    callback(400, {
                      Error: "The hostname didn't resolve to any DNS name",
                    });
                  });
  ...
  index: (data, callback) => {
    ...
    const templateData = {
      ...
      "head.description":
        "Monitors website and alerts user with a SMS when state changes happen",
      "body.class": "index",
    };
