...
const handlers = {
              ...
              _data.delete("users", phone, (error) => {
                if(!error) {
                  ...
                  if(deletions === userChecksTotal && !deletionErrors) {
                    ...
                    callback(500, { Error: "Something went wrong when deleting the checks" });
                  }
  ...
  _checks: {
    post: (data, callback) => {
      ...
      const timeoutSeconds = (typeof data.payload.timeoutSeconds === "number" && data.payload.timeoutSeconds > 0 && data.payload.timeoutSeconds < 6) ? data.payload.timeoutSeconds : false;
                  ...
                  _data.create("checks", checkId, checkObject, (err) => {
                    if(!err) {
                      ...
                      _data.update("users", phone, userData, (err) => {
                        ...
                        callback(500, { Error: "Failed to update user with a new check" })
                      });
    ...
    get: (data, callback) => {
      const id = typeof (data.query.id) === "string" ? data.query.id.trim() : false;
      ...
      if( protocol || url || method || successCodes || timeoutSeconds ) {
        _data.read("checks", id, (err, checkData) => {
          if(!err && data) {
            ...
            handlers._tokens.verifyToken(token, checkData.userPhone, (isValidToken) => {
              if(isValidToken) {
                const updatableData = { protocol, url, method, successCodes, timeoutSeconds };
  ...
  public: (data, callback) => {
    ...
    helpers.getStaticAsset(trimmedAssetName, (err, data) => {
      if(!err && data) {
        ...
        if(trimmedAssetName.contains(".css")) {
          contentType = "css";
        } else if(trimmedAssetName.contains(".js")) {
          contentType = "js";
        } else if(trimmedAssetName.contains(".png")) {
          contentType = "png";
        } else if(trimmedAssetName.contains(".jpg")) {
          contentType = "jpg";
        } else if(trimmedAssetName.contains(".ico")) {
          contentType = "favicon";
        }
