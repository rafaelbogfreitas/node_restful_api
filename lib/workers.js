...
const workers = {
      ...
      if(!err && checks.length) {
        checks.forEach(check => {
          _data.read("checks", check, (err, originalCheck) => {
            ...
            debug(`Could not find the check ${check}`);
          });
        });
        return;
      }
      debug("No checks found");
    });

    debug("\x1b[35m%s\x1b[0m", "Gathering checks...");
  },
  validateCheckData: (checkData) => {
    ...
    debug("Error: one of the checks isn't formatted properly");
  },
    ...
    _data.update("checks", newCheckData.id, newCheckData, (err) => {
      if(!err) {
        if(alertUser) {
          ...
          debug("Check has not changed. User no alerted");
        }
        return;
      }
      debug(`Error updating check: ${checkData.id}`);
    });
  },
  ...
  alerUserToStatusChange: ({ userPhone, method, protocol, url, state }) => {
    ...
    helpers.sendTwilioSMS(userPhone, msg, (err) => {
      if(!err) {
        debug("User alerted successfully");
        return;
      }
      debug(`Error sending SMS to user. Error: ${err}`);
    });
  },
          ...
          _logs.compress(logId, newFileId, (err) => {
            if(!err) {
              _logs.truncate(logId, (err) => {
                if(!err) {
                  debug("Successfully truncate file");
                  return;
                }
                debug("Error truncating the file");
              })
              return;
            }
            debug("Error compressing the file");
          })
        });
